...
const Login = () => {
    ...
    const handleRegister = async (e) => {
        ...
        try {
            ...
            if (error.response && error.response.data) {
                showAlert(error.response.data.message || 'Register failed', 'error');
            } else {
                showAlert('Error', 'error');
            }
            console.error('Error:', error);
    ...
    return (
        <div className="flex items-center justify-center min-h-screen">
            {alertMessage && (
                <div
                    className={`fixed top-0 mx-auto transform ${
                        alertType === 'error'
                            ? "bg-red-100 border border-red-400 text-red-700"
                            : "bg-green-100 border border-green-400 text-green-700"
                    } py-3 px-4 rounded-lg mb-4 w-80 max-w-md text-center z-50 animate-slide-down`}
                    role="alert"
                >
                    <span className="text-center flex justify-center w-full items-center space-x-2">
                        {alertType === 'error' ? <FaExclamationCircle className="w-4 h-4" /> : ''}
